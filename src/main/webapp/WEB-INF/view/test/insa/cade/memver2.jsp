...
<!-- ------------------------------------------------------------- -->		           
				          <tr id="school" style="display:none;">		           
		                 	<td>
		                 		<input type="text" id="sch_no" name="sch_no" style="display:none;">
		                 		
		                 	</td>
			                <td>
			                     <input type="text" class="form-control col-lg-1" style="width: 50%;" placeholder="학교이름" name="sch_name" value="test">
		...
		<c:forEach items="${school}" var="memver">
		                 <tr id="${memver.sch_no}">
			                ...
			                <td>
			                     <input type="text" class="form-control col-lg-1" style="width: 50%;" name="sch_name" value="${memver.sch_name}">
			                      <select  id="disabledSelect" class="form-control col-lg-1"  style="width: 50%;" name="sch_grade">
...
<!-- ------------------------------------------------------------- -->	
 ...
 /* 	 	var sch_no = []
		...
		var sch_endday = []
		var sch_status = []
	  <c:forEach items="${school}" var="memver" begin="2" end="100">
	 	sch_no.push("${memver.sch_no}");
		sch_name.push("${memver.sch_name}")
	 	sch_grade.push("${memver.sch_grade}")
	 	sch_major.push("${memver.sch_major}")
	 	sch_startday.push("${memver.sch_startday}")
	 	sch_endday.push("${memver.sch_endday}")
	 	sch_status.push("${memver.sch_status}")
	 </c:forEach> 
		
		
	 	for (var i = 0; i < sch_no.length; i++) {
	 	    //console.log(sch_no[i]);
	 	    
	 	   //복사
	 	   var d = $('#school').clone(true)
	 	   console.log($("#sch_no").html());
	 	   	//삽입
	 		 $('#schoolsit').append(
	 				 d.removeClass('#school').attr('id', sch_no[i]).show()
	 				 )
	 	}  */
...
<!-- ------------------------------------------------------------- -->		           
	          	 <tr id="jagyeo" style="display:none;">		     
	                   ...
	                   <td><input type="text" class="form-control" name="cer_name" value="test"></td>
